...
const ServerItem = ({ color, title, teamId, id, bar, online, onClickServer }) => {
    ...
    <div className={cx('serveritem-back')}>
      {bar === true ?
        <div className={cx('serveritem-contents')}>
          ...
          <div className={cx('serveritem-circle')} id={teamId} style={{backgroundColor: color, color: color}} title={title} onClick={onClickServer}>{id}</div>
        </div>
        :
        <div className={cx('serveritem-contents')}>
          { online === true ?
